...
/**
 * [UnknownValueReader] which reads the first value whose tag matches [targetTag]. The value will
 ...
 */
class SingleUnknownValueReader<M : Message<M, *>, V : Any>(
  private val targetTag: Int,
  private val adapter: ProtoAdapter<V>,
) : UnknownValueReader<M, V?> {
  override fun read(message: M): V? {
    val reader = ProtoReader(Buffer().write(message.unknownFields))
    reader.forEachTag { tag ->
      if (tag != targetTag) {
        reader.skip()
      } else {
        return adapter.decode(reader)
      }
    }
    return null
  }
