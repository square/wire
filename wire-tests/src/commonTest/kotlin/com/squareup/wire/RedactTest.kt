...
class RedactTest {
  @Test fun string() {
    ...
    assertThat(redacted.toString()).isEqualTo<String>("RedactedFields{a=██, b=b, c=c}")
    val redactedRepeated = RedactedRepeated(
      a = listOf("a", "b"),
      b = listOf(RedactedFields("a", "b", "c", null), RedactedFields("d", "e", "f", null)),
    )
    assertThat(redactedRepeated.toString()).isEqualTo<String>(
  ...
  @Test fun requiredRedactedFieldThrowsRedacting() {
    ...
    try {
      ...
      assertThat(e.message).isEqualTo<String?>("Field 'a' is required and cannot be redacted.")
    }
  }

  @Test fun requiredRedactedFieldToString() {
    val adapter = RedactedRequired.ADAPTER
    assertThat(adapter.toString(RedactedRequired("a"))).isEqualTo<String>("RedactedRequired{a=██}")
  }
  ...
  @Test fun requiredRedactedFieldToString_buildersOnly() {
    ...
    ).isEqualTo<String>("RedactedRequired{a=██}")
  }
}
