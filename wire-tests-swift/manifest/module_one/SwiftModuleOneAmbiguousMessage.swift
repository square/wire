...
extension SwiftModuleOneAmbiguousMessage : Proto2Codable {
    ...
    public init(from protoReader: ProtoReader) throws {
        ...
        var addressProtoData: Foundation.Data? = nil
        var locationProtoData: Foundation.Data? = nil

        let token = try protoReader.beginMessage()
        while let tag = try protoReader.nextTag(token: token) {
            switch tag {
            case 1: name = try protoReader.decode(String.self)
            case 2: try protoReader.decodeMessage(into: &addressProtoData)
            case 3: try protoReader.decodeMessage(into: &locationProtoData)
            default: try protoReader.readUnknownField(tag: tag)
            }
        }
        if let addressProtoData {
            address = try protoReader.decodeMergedMessage(module_address.Address.self, from: addressProtoData)
        }
        if let locationProtoData {
            location = try protoReader.decodeMergedMessage(Location.self, from: locationProtoData)
        }
