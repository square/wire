...
// Source: squareup.protos.kotlin.swift_modules.module_two.SwiftModuleTwoMessage in swift_module_two.proto
import Foundation
import Wire
import module_one
...
extension SwiftModuleTwoMessage : ProtoMessage {
    ...
    public static func protoMessageTypeURL() -> String {
        return "type.googleapis.com/squareup.protos.kotlin.swift_modules.module_two.SwiftModuleTwoMessage"
    }
...
extension SwiftModuleTwoMessage.NestedMessage : ProtoMessage {
    ...
    public static func protoMessageTypeURL() -> String {
        return "type.googleapis.com/squareup.protos.kotlin.swift_modules.module_two.SwiftModuleTwoMessage.NestedMessage"
    }
...
extension SwiftModuleTwoMessage.NestedMessage : Proto2Codable {
    ...
    public init(from protoReader: ProtoReader) throws {
        ...
        var module_typeProtoData: Foundation.Data? = nil

        let token = try protoReader.beginMessage()
        while let tag = try protoReader.nextTag(token: token) {
            switch tag {
            case 1: try protoReader.decode(into: &array_types)
            case 2: try protoReader.decodeMessage(into: &module_typeProtoData)
            default: try protoReader.readUnknownField(tag: tag)
            }
        }
        if let module_typeProtoData {
            module_type = try protoReader.decodeMergedMessage(SwiftModuleOneMessage.self, from: module_typeProtoData)
        }
        self.unknownFields = try protoReader.endMessage(token: token)
