...
extension Screen : Proto2Codable {
    ...
    public init(from protoReader: ProtoReader) throws {
        ...
        var screenProtoTag: UInt32? = nil
        var screen_oneof_sub_messageProtoData: Foundation.Data? = nil
        var selfProtoData: Foundation.Data? = nil
        var view: View? = nil

        let token = try protoReader.beginMessage()
        while let tag = try protoReader.nextTag(token: token) {
            switch tag {
            case 1: screenProtoTag = 1; screen = .screen_oneof_string(try protoReader.decode(String.self))
            case 2: screenProtoTag = 2; screen = .screen_oneof_int32(try protoReader.decode(Int32.self))
            case 3: if screenProtoTag != 3 { screen_oneof_sub_messageProtoData = nil }; screenProtoTag = 3; try protoReader.decodeMessage(into: &screen_oneof_sub_messageProtoData)
            case 4: if screenProtoTag != 4 { selfProtoData = nil }; screenProtoTag = 4; try protoReader.decodeMessage(into: &selfProtoData)
        ...
        if screenProtoTag == 3, let screen_oneof_sub_messageProtoData {
            screen = .screen_oneof_sub_message(try protoReader.decodeMergedMessage(Screen.SubMessage.self, from: screen_oneof_sub_messageProtoData))
        }
        if screenProtoTag == 4, let selfProtoData {
            screen = .`self`(try protoReader.decodeMergedMessage(Screen.Self_.self, from: selfProtoData))
        }
...
extension Screen.SubMessage : Proto2Codable {
    ...
    public init(from protoReader: ProtoReader) throws {
        ...
        var submessageProtoTag: UInt32? = nil
        var submessage_oneof_nested_sub_messageProtoData: Foundation.Data? = nil
        ...
        while let tag = try protoReader.nextTag(token: token) {
            ...
            case 3: submessageProtoTag = 3; submessage = .submessage_oneof_string(try protoReader.decode(String.self))
            case 4: if submessageProtoTag != 4 { submessage_oneof_nested_sub_messageProtoData = nil }; submessageProtoTag = 4; try protoReader.decodeMessage(into: &submessage_oneof_nested_sub_messageProtoData)
            default: try protoReader.readUnknownField(tag: tag)
            }
        }
        if submessageProtoTag == 4, let submessage_oneof_nested_sub_messageProtoData {
            submessage = .submessage_oneof_nested_sub_message(try protoReader.decodeMergedMessage(Screen.SubMessage.NestedSubMessage.self, from: submessage_oneof_nested_sub_messageProtoData))
        }
        self.unknownFields = try protoReader.endMessage(token: token)
