...
extension MessageUsingMultipleEnums : Proto2Codable {
    ...
    public init(from protoReader: ProtoReader) throws {
        ...
        while let tag = try protoReader.nextTag(token: token) {
            switch tag {
            case 1: if let value = try protoReader.decode(MessageWithStatus.Status.self) { a = value }
            case 2: if let value = try protoReader.decode(OtherMessageWithStatus.Status.self) { b = value }
