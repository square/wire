...
extension OuterMessage : Proto2Codable {
    ...
    public init(from protoReader: ProtoReader) throws {
        ...
        var embedded_messageProtoData: Foundation.Data? = nil

        let token = try protoReader.beginMessage()
        while let tag = try protoReader.nextTag(token: token) {
            switch tag {
            case 1: outer_number_before = try protoReader.decode(Int32.self, encoding: .variable)
            case 2: try protoReader.decodeMessage(into: &embedded_messageProtoData)
            default: try protoReader.readUnknownField(tag: tag)
            }
        }
        if let embedded_messageProtoData {
            embedded_message = try protoReader.decodeMergedMessage(EmbeddedMessage.self, from: embedded_messageProtoData)
        }
