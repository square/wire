...
extension ContainsDuration : Proto3Codable {
    ...
    public init(from protoReader: ProtoReader) throws {
        ...
        var durationProtoData: Foundation.Data? = nil

        let token = try protoReader.beginMessage()
        while let tag = try protoReader.nextTag(token: token) {
            switch tag {
            case 1: try protoReader.decodeMessage(into: &durationProtoData)
            default: try protoReader.readUnknownField(tag: tag)
            }
        }
        if let durationProtoData {
            duration = try protoReader.decodeMergedMessage(Duration.self, from: durationProtoData)
        }
