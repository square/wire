...
extension ContainsTimestamp : Proto3Codable {
    ...
    public init(from protoReader: ProtoReader) throws {
        ...
        var timestampProtoData: Foundation.Data? = nil

        let token = try protoReader.beginMessage()
        while let tag = try protoReader.nextTag(token: token) {
            switch tag {
            case 1: try protoReader.decodeMessage(into: &timestampProtoData)
            default: try protoReader.readUnknownField(tag: tag)
            }
        }
        if let timestampProtoData {
            timestamp = try protoReader.decodeMergedMessage(Timestamp.self, from: timestampProtoData)
        }
