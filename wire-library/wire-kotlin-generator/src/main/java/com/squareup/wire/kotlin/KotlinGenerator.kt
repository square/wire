            ...
            for (fieldOrOneOf in message.fieldsAndFlatOneOfFieldsAndBoxedOneOfs()) {
              when (fieldOrOneOf) {
                ...
                is OneOf -> {
                  ...
                  newName(
                    boxedOneOfClassName(fieldOrOneOf.name), boxedOneOfClassName(fieldOrOneOf.name)
                  )
  ...
  private fun generateNewBuilderMethod(type: MessageType, builderClassName: ClassName): FunSpec {
    ...
    if (!javaInterOp) {
      return funBuilder
        ...
        .returns(NOTHING)
        .addStatement(
          "throw %T(%S)", ClassName("kotlin", "AssertionError"),
        ...
        when (fieldOrOneOf) {
          is Field -> {
            ...
            val throwExceptionBlock = if (!fieldOrOneOf.isRepeated && fieldOrOneOf.isRequired) {
              CodeBlock.of(
                " ?: throw %1M(%2L, %2S)", missingRequiredFields, nameAllocator[fieldOrOneOf]
              )
            } else {
              CodeBlock.of("")
            }
