...
sealed class Target : Serializable {
  ...
  interface SchemaHandler {
    ...
    fun handle(
      ...
      claimedPaths: MutableMap<Path, Type>,
      isExclusive: Boolean
    ) {
      protoFile.types
          ...
          .forEach { type ->
            ...
            if (generatedFilePath != null) {
              if (generatedFilePath in claimedPaths.keys) {
                val other = claimedPaths[generatedFilePath]!!
                throw IllegalStateException(
                    "Same type is getting generated by different messages:\n" +
                        "  ${other.type.simpleName} at ${other.location}\n" +
                        "  ${type.type.simpleName} at ${type.location}")
              }
              claimedPaths[generatedFilePath] = type
            }
    ...
    return object : SchemaHandler {
      override fun handle(type: Type): Path? {
        ...
        val generatedFilePath =
            fs.getPath(outDirectory, javaFile.packageName, javaFile.typeSpec.name)
    ...
    return object : SchemaHandler {
      override fun handle(type: Type): Path? {
        ...
        val kotlinFile = FileSpec.builder(className.packageName, typeSpec.name!!)
            ...
            .addType(typeSpec)
            .build()
        val generatedFilePath = fs.getPath(outDirectory, kotlinFile.packageName, kotlinFile.name)
