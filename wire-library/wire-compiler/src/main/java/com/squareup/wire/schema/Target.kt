...
sealed class Target : Serializable {
  ...
  interface SchemaHandler {
    ...
    fun handle(
      ...
      claimedTypePaths: MutableMap<Path, Type>,
      claimedServicePaths: MutableMap<Path, Service>,
      isExclusive: Boolean
    ) {
      protoFile.types
          ...
          .forEach { type ->
            ...
            if (generatedFilePath != null) {
              claimedTypePaths.putIfAbsent(generatedFilePath, type)?.let { other ->
      ...
      protoFile.services
          ...
          .forEach { service ->
            ...
            for (generatedFilePath in generatedFilePaths) {
              claimedServicePaths.putIfAbsent(generatedFilePath, service)?.let { other ->
                throw IllegalStateException(
                    "Same file is getting generated by different services:\n" +
                        "  ${other.type().simpleName} at ${other.location()}\n" +
                        "  ${service.type().simpleName} at ${service.location()}")
              }
            }
