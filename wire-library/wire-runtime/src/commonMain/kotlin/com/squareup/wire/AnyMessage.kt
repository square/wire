 ...
 * [value] is the serialized representation of the wrapped message. The [toAny] and [unpack] methods
 * are available to wrap and unwrap an `Any` message.
 *
 * Example
 * ```
 * val anyMessage: AnyMessage = person.toAny()
...
class AnyMessage(
  ...
  companion object {
    @JvmName("of")
    @JvmStatic
    fun Message<*, *>.toAny(): AnyMessage {
      val typeUrl = adapter.typeUrl
          ?: error("recompile ${this::class} to use it with AnyMessage")
      return AnyMessage(typeUrl, encodeByteString())
    }
